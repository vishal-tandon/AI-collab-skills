---
name: reflect
description: Run a reflection on work done with the user. Invoke when the user types /reflect, "reflect on this", "reflect on the last task", "what did we learn", "what went well", "let's reflect", or after a task or feature is completed and the user wants to capture learnings. Two modes: full session (default) or last task only. Analyse the conversation autonomously — no questions to the user. Surface what worked, what didn't, what would have been faster, and how the user can prompt more effectively. Always show proposed memory saves before writing them.
---
...
**5. What works with the user vs what doesn't**
...
- What does the user never do? (e.g. never gives full spec upfront, never asks for explanations, never wants options he didn't ask for)
- What should Claude pre-empt next time this type of task comes up?
- What is the user's role in the loop — curator/director who tunes visually, not spec-writer?
- What delivery model fits best — sandbox with controls → final values → clean embed?
- If a new collaborator joined and asked "how does the user like to work?", what would you tell them from this session alone?
...
- Explicitly hunt for confirmed working methods that lack a memory entry — not just corrections, but positive patterns that reliably produce good outcomes with the user and should be locked in so future sessions don't drift
...
- Was /wrap suggested proactively, or did the user have to ask?
...
| Type | Question |
|------|----------|
| **Process** | Was there a repeatable sequence of steps done manually that needs enforcing? (Signal: Claude or the user had to remember what came next, or steps were done out of order.) |
...
| **Quality gate** | Was there a check that needed to happen before proceeding but was skipped, forgotten, or inconsistently applied? (Signal: the user caught something Claude should have caught first.) |
...
- How the user gives feedback, how he works
